...
function SandboxTab() {
  ...
  const [activeSbId, setActiveSbId] = useState(null);
  const [input, setInput] = useState('');
  const [building, setBuilding] = useState(false);
  const [pendingImages, setPendingImages] = useState([]); // [{name, dataUrl}]
  ...
  useEffect(() => {
    if (!activeSbId || !building) {
      if (pollRef.current) { clearInterval(pollRef.current); pollRef.current = null; }
      return;
    }
    pollRef.current = setInterval(async () => {
      try {
        const r = await fetch(`/api/sandbox/${activeSbId}`);
        const d = await r.json();
        if (d.id) {
          setSandboxes(prev => ({ ...prev, [d.id]: d }));
          if (d.status !== 'building') setBuilding(false);
        }
      } catch {}
    }, 2000);
  ...
  }, [sandboxes[activeSbId]?.messages?.length, building]);

  async function sendMessage() {
    if ((!input.trim() && pendingImages.length === 0) || building) return;
    setBuilding(true);

    let sbId = activeSbId;

    // Create new sandbox if none active
    if (!sbId) {
      const cr = await fetch('/api/sandbox', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: input.slice(0, 60) || 'New Sandbox' }),
      });
      const cd = await cr.json();
      sbId = cd.sandbox.id;
      setSandboxes(prev => ({ ...prev, [sbId]: cd.sandbox }));
      setActiveSbId(sbId);
    }
    ...
    await fetch(`/api/sandbox/${sbId}/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  function handleImageSelect(e) {
    ...
    files.forEach(file => {
      ...
      reader.onload = ev => {
        setPendingImages(prev => [...prev, { name: file.name, dataUrl: ev.target.result }]);
      };
  ...
  async function deleteSandbox(id) {
    ...
    if (activeSbId === id) setActiveSbId(null);
  }

  const sb = activeSbId ? sandboxes[activeSbId] : null;
  const sbList = Object.values(sandboxes).sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

  const logStyle = {
    background: '#0D0D0D', color: '#6CEFA0', fontFamily: T.mono, fontSize: '0.68rem',
    lineHeight: 1.55, padding: '0.75rem', maxHeight: '200px', overflowY: 'auto',
    whiteSpace: 'pre-wrap', wordBreak: 'break-all', borderRadius: T.radius,
    border: '1px solid rgba(108,239,160,0.15)',
  };

  // Render a single message content (string or array of blocks)
      ...
      return content.map((block, i) => {
        ...
        if (block.type === 'image') {
          const src = block.source?.type === 'base64'
            ...
            : block.source?.url;
          return <img key={i} src={src} alt="uploaded" style={{ maxWidth: '100%', maxHeight: 200, borderRadius: T.radius, marginTop: '0.5rem', display: 'block' }} />;
        }
        return null;
      });
    }
  ...
  return (
    <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', height: 'calc(100vh - 160px)', minHeight: 600 }}>
      {/* Left sidebar: sandbox list */}
      <div style={{ width: 260, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        <div style={S.section}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <div style={S.sectionTitle}>Sandboxes</div>
            <button
              onClick={() => setActiveSbId(null)}
              style={{ ...S.btn, fontSize: '0.65rem', padding: '0.2rem 0.5rem' }}
            >+ New</button>
          </div>
          {sbList.length === 0 && (
            <div style={{ color: T.muted, fontSize: '0.72rem', fontFamily: T.mono, padding: '0.5rem 0' }}>
              No sandboxes yet. Describe an app to build.
            </div>
          )}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            {sbList.map(s => (
              <div key={s.id}
                onClick={() => setActiveSbId(s.id)}
                style={{
                  ...S.card, cursor: 'pointer', padding: '0.5rem 0.65rem',
                  border: activeSbId === s.id ? `1px solid ${T.blue}` : S.card.border,
                  background: activeSbId === s.id ? `${T.blue}18` : T.card,
                }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.75rem', fontFamily: T.mono, color: T.text, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {s.title}
                  </span>
                  <button
                    onClick={e => { e.stopPropagation(); deleteSandbox(s.id); }}
                    style={{ background: 'none', border: 'none', color: T.muted, cursor: 'pointer', fontSize: '0.85rem', padding: '0 2px', marginLeft: '0.4rem' }}>
                    ×
                  </button>
                </div>
                <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.2rem', alignItems: 'center' }}>
                  <span style={{ ...S.badge(s.status === 'deployed' ? T.mint : s.status === 'building' ? T.orange : T.blue), fontSize: '0.58rem' }}>
                    {s.status}
                  </span>
                  {s.url && (
                    <a href={s.url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}
                      style={{ color: T.muted, fontSize: '0.62rem', fontFamily: T.mono, textDecoration: 'none' }}>
                      :{s.port} ↗
                    </a>
                  )}
                </div>
      ...
      {/* Center: chat + input */}
      <div style={{ width: 380, flexShrink: 0, display: 'flex', flexDirection: 'column', height: '100%' }}>
        {/* Chat history */}
        <div style={{
          flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem',
          padding: '0.75rem', background: T.bg, borderRadius: T.radius, border: T.border, marginBottom: '0.75rem',
          minHeight: 300,
        }}>
          {!sb && (
            <div style={{ color: T.muted, fontFamily: T.mono, fontSize: '0.75rem', textAlign: 'center', margin: 'auto' }}>
              Start a new sandbox by describing an app below
            </div>
          )}
          {sb && (sb.messages || []).map((msg, i) => {
            const isUser = msg.role === 'user';
            return (
              <div key={i} style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start' }}>
                <div style={{
                  maxWidth: '85%', padding: '0.5rem 0.75rem', borderRadius: T.radius,
                  background: isUser ? `${T.blue}22` : `${T.card}`,
                  border: isUser ? `1px solid ${T.blue}44` : T.border,
                  fontSize: '0.78rem', fontFamily: T.mono, color: T.text, lineHeight: 1.5,
          ...
          {building && (
            <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
              <div style={{ padding: '0.5rem 0.75rem', borderRadius: T.radius, background: T.card, border: T.border, fontSize: '0.75rem', fontFamily: T.mono, color: T.orange }}>
                building...
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.5rem' }}>
            {pendingImages.map((img, i) => (
              <div key={i} style={{ position: 'relative' }}>
                <img src={img.dataUrl} alt={img.name} style={{ width: 56, height: 56, objectFit: 'cover', borderRadius: T.radius, border: T.border }} />
                <button
                  onClick={() => setPendingImages(prev => prev.filter((_, j) => j !== i))}
                  style={{ position: 'absolute', top: -6, right: -6, background: T.red, border: 'none', color: '#fff', borderRadius: '50%', width: 16, height: 16, fontSize: '0.6rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                  ×
                </button>
              </div>
            ))}
        ...
        {/* Input area */}
        <div style={{ ...S.card, padding: '0.75rem', flexShrink: 0 }}>
          <textarea
            value={input}
            onChange={e => setInput(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) sendMessage(); }}
            placeholder={sb ? 'Ask to modify the app, add features, change UI...' : 'Describe an app to build and deploy...'}
            disabled={building}
            rows={3}
            style={{
              width: '100%', background: T.bg, border: T.border,
              borderRadius: T.radius, padding: '0.5rem', color: T.text,
              fontFamily: T.mono, fontSize: '0.78rem', resize: 'none',
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            ...
            <button
              ...
              style={{ ...S.btn, padding: '0.3rem 0.6rem', fontSize: '0.72rem', background: 'transparent', border: T.border }}
              title="Upload images">
              📎
            </button>
            <button
              onClick={sendMessage}
              disabled={(!input.trim() && pendingImages.length === 0) || building}
              style={{ ...S.btn, flex: 1, opacity: ((!input.trim() && pendingImages.length === 0) || building) ? 0.5 : 1 }}
            >
              {building ? 'Building...' : (sb ? 'Send' : 'Build Sandbox')}
            </button>
          </div>
        </div>
      </div>

      {/* Right: preview + log + workers */}
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '0.75rem', height: '100%', overflowY: 'auto' }}>
        {sb ? (
          <>
            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontFamily: T.mono, fontSize: '0.85rem', color: T.text, fontWeight: 600 }}>{sb.title}</div>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                ...
                {sb.url && (
                  <a href={sb.url} target="_blank" rel="noreferrer"
                    style={{ color: T.blue, fontSize: '0.7rem', fontFamily: T.mono, textDecoration: 'none' }}>
                    :{sb.port} ↗
                  </a>
                )}
                {(sb.log || []).length > 0 && (
                  <button onClick={() => setShowLog(l => !l)} style={{ ...S.btn, fontSize: '0.65rem', padding: '0.2rem 0.5rem', background: 'transparent', border: T.border }}>
                    {showLog ? 'Hide Log' : 'Build Log'}
                  </button>
                )}
              </div>
            </div>
            ...
            {showLog && (sb.log || []).length > 0 && (
              <div style={logStyle} ref={el => el && (el.scrollTop = el.scrollHeight)}>
                {(sb.log || []).map((entry, i) => (
                  <div key={i} style={{ marginBottom: '0.2rem' }}>
                    <span style={{ color: entry.tool === 'error' ? T.red : entry.tool === 'user' ? T.blue : T.orange }}>[{entry.tool}]</span>{' '}
                    <span style={{ color: '#A0FAB8' }}>{entry.result}</span>
                  </div>
                ))}
                {building && <span style={{ color: T.orange }}>▌</span>}
              </div>
            )}

            {/* Live Preview via proxy (fixes mixed content) */}
          ...
          <div style={{ ...S.card, textAlign: 'center', padding: '3rem', color: T.muted, fontFamily: T.mono, fontSize: '0.82rem', margin: 'auto' }}>
            Select or create a sandbox on the left
          </div>
        )}
      </div>
    </div>
